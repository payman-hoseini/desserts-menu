...
export default function Cart (){
    ...
    return (
        <>
                ...
                <div className={`flex flex-col mb-5 ${(total==0) ? "items-center mt-8" : "mt-1"}`}>
                    {
                        ...
                        <>
                            <div className="flex justify-between items-center py-4 border-b-Rose-100 border-b">
                                <div>
                                    <p className="font-semibold text-sm lg:text-base text-Rose-900">{p.product.name}</p>
                                    <div className="space-x-3">
                                        <span className="text-Red text-xs md:text-base font-semibold">{p.count}x</span>
                                        <span className="text-Rose-400 text-xs md:text-base">@ ${p.product.price}</span>
                                        <span className="text-Rose-500 text-xs md:text-base font-semibold">${p.count * parseFloat(p.product.price)}</span>
                                    </div>
                                </div>
                    ...
                    }
                    {
                        ...
                        <>
                            <div className="mt-4 flex justify-between items-center">
                                <p className="text-Rose-900 text-sm lg:text-base">Order Total</p>
                                <p className="font-bold text-2xl lg:text-3xl">${sum}</p>
                            </div>
                            <div>
                                <div>
                                    <div className="flex items-center justify-center mt-7 bg-Rose-50 py-4 rounded-md">
                                        ...
                                        <p className="ml-2 text-xs md:text-[10px] lg:text-base">This is a <span className="font-semibold">carbon-neutral</span> delivery</p>
                                    </div>
                                    <button onClick={() => {setShowModal(true)}} className="bg-Red text-white text-sm lg:text-base font-semibold w-full mt-6 py-4 rounded-full hover:bg-red-800">
                                        Confirm Order
                                    </button>
                                </div>
                            </div>
            ...
            {
                showModal && 
                <div ref={modalRef} className="fixed z-10 left-0 top-0 w-full h-full bg-black bg-opacity-50 grid md:place-content-center">
                    <div className="bg-white py-8 mt-20 px-8 md:rounded-lg rounded-t-xll]">
                        <div>
                        ...
                        <div className="mt-5">
                            <h2 className="text-4xl md:text-3xl font-bold text-Rose-900">Order Confirmed</h2>
                            <p className="text-Rose-400 md:text-xs text-sm font-semibold mt-1">We hope you enjoy your food!</p>
